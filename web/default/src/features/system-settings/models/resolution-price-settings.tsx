...
function rowsToObject(rows: ResolutionPriceRow[]): Record<string, number> {
...
function objectToRows(prices: Record<string, number>): ResolutionPriceRow[] {
...
type ResolutionPriceSettingsProps = {
  defaultValue: string
}

export const ResolutionPriceSettings = memo(function ResolutionPriceSettings({
  defaultValue,
}: ResolutionPriceSettingsProps) {
  const { t } = useTranslation()
  const updateOption = useUpdateOption()
  const [editMode, setEditMode] = useState<'visual' | 'json'>('visual')
  const [rows, setRows] = useState<ResolutionPriceRow[]>([])
  ...
  const syncFromRows = useCallback((nextRows: ResolutionPriceRow[]) => {
    setRows(nextRows)
  ...
  const addRow = useCallback(() => {
    const newRow: ResolutionPriceRow = { id: nextRowId, key: '', price: 0 }
    ...
    <div className='space-y-4'>
      ...
      {editMode === 'visual' ? (
        <StaticDataTable
          ...
          emptyContent={t('No resolutions configured')}
          columns={[
            {
              id: 'resolution',
              header: t('Resolution'),
